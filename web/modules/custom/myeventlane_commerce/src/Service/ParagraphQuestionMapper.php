...
class ParagraphQuestionMapper {
  ...
  public function buildElements(EntityInterface $event, array $defaults = []): array {
    ...
    foreach ($event->get('field_attendee_questions')->referencedEntities() as $para) {
      $machine = $para->get('field_machine_name')->value ?? strtolower(preg_replace('/\s+/', '_', $para->label()));
      $type = $para->get('field_widget_type')->value ?? 'textfield';
      $required = (bool) ($para->get('field_required')->value ?? FALSE);
      $title = $para->label();
      $help = $para->get('field_help')->value ?? '';

      switch ($type) {
        case 'select':
          $options = $this->explodeOptions($para->get('field_options')->value ?? '');
          $elements[$machine] = [
            '#type' => 'select',
            '#title' => $title,
            '#options' => $options,
