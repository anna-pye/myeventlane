    ...
    if ($event && $event->hasField('field_attendee_questions')) {
      foreach ($event->get('field_attendee_questions') as $delta => $ref) {
        ...
        if ($para instanceof Paragraph) {
          $label = ($para->hasField('field_label') && !$para->get('field_label')->isEmpty() ? $para->get('field_label')->value : '') ?? '';
          $type = '';
          if ($entity instanceof \Drupal\Core\Entity\ContentEntityInterface && $entity->hasField('field_type') && !$entity->get('field_type')->isEmpty()) {
            $type = $entity->get('field_type')->value;
          }
